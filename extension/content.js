(()=>{
 ...
 async function sendPart(text){const box=composer();if(!box)throw new Error('O campo de mensagem do WhatsApp não foi encontrado. Atualize a página e tente novamente.');if(box.textContent?.trim())throw new Error('Há um texto sendo digitado pelo vendedor. A IA foi pausada para não sobrescrevê-lo.');box.focus();document.execCommand('insertText',false,text);box.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:text}));await wait(Math.min(2200,650+text.length*8));const button=sendButton();if(!button)throw new Error('O botão Enviar não foi encontrado. O WhatsApp Web pode ter mudado.');button.click();await wait(950);}
