(()=>{
 ...
 const runtime={contactKey:'',contactName:'',active:false,tracked:false,busy:false,sending:false,ready:false,lastFingerprint:'',summary:'',needsHuman:false,reason:'',pending:null,lastScan:0,lastStatus:0,lastOutbox:0,error:'',renderKey:'',notifiedKey:'',autoTriedKey:'',dashboardUrl:''};
 ...
 function render(){
  ...
  const key=JSON.stringify([runtime.contactKey,runtime.active,runtime.tracked,runtime.busy,runtime.sending,runtime.summary,runtime.needsHuman,runtime.reason,runtime.pending?.requestMessageId,runtime.error,runtime.service]);if(runtime.renderKey===key)return;runtime.renderKey=key;
 ...
 function openPanel(){let panel=document.querySelector('#retoma-panel');if(panel){panel.remove();document.querySelector('#retoma-launcher').hidden=false;return;}runtime.renderKey='';panel=document.createElement('aside');panel.id='retoma-panel';panel.innerHTML=`<header><img src="${icon}" alt="Símbolo Retoma"><div><b>retoma</b><small id="retoma-contact">Abra uma conversa</small></div><button type="button" aria-label="Fechar">×</button></header><div class="retoma-panel-body"><div class="retoma-status" id="retoma-status">Conectando…</div><div id="retoma-dynamic"></div><a class="retoma-dashboard" href="${escape(runtime.dashboardUrl||'https://retoma-ai.vercel.app')}" target="_blank">Abrir central de atendimento <span>↗</span></a><footer><span></span>Versão 0.6.2 · mantenha esta aba aberta</footer></div>`;document.body.appendChild(panel);document.querySelector('#retoma-launcher').hidden=true;panel.querySelector('header button').onclick=openPanel;render();}
 async function hydrate(force=false){
  ...
  try{const data=await api({mode:'status',contactKey:current.key,contactName:current.name});applyState(data);Object.assign(runtime,{ready:true,lastStatus:Date.now()});await remember(data.active,data.service||'');render();if(data.pending)await deliver(data.pending);else if(data.active&&transcript().at(-1)?.role==='cliente')void processCurrent();else if(!data.tracked)void maybeAutoActivate();}catch(e){runtime.ready=true;runtime.lastStatus=Date.now();showError(e.message);}
 }
 ...
 async function maybeAutoActivate(){
  if(!runtime.ready||runtime.tracked||runtime.busy||runtime.autoTriedKey===runtime.contactKey)return;const items=transcript(),last=items.at(-1);if(!last||last.role!=='cliente')return;
 ...
 async function deliver(reply){
  if(runtime.sending||runtime.lastDeliveredRequest===reply.requestMessageId)return;runtime.lastDeliveredRequest=reply.requestMessageId;runtime.sending=true;runtime.busy=true;runtime.pending=reply;render();const expected=runtime.contactKey;
  try{const settings=await chrome.storage.local.get(['assistantEnabled','sendAsBlocks','responseDelaySeconds']);if(reply.sender!=='vendedor'&&settings.assistantEnabled===false){runtime.lastDeliveredRequest='';return;}const parts=settings.sendAsBlocks===false?[reply.text]:(reply.parts?.length?reply.parts:[reply.text]);const delaySeconds=Math.max(0,Math.min(15,Number(settings.responseDelaySeconds??2)));if(delaySeconds)await wait(delaySeconds*1000);for(const part of parts){if(identity()?.key!==expected)throw new Error('A conversa mudou antes do envio. Volte ao contato para continuar.');await sendPart(part);}await api({mode:'sent',contactKey:expected,contactName:runtime.contactName,requestMessageId:reply.requestMessageId,messages:transcript()});runtime.pending=null;runtime.summary=reply.summary||runtime.summary;runtime.needsHuman=reply.needsHuman;runtime.reason=reply.reason||'';runtime.active=reply.sender==='vendedor'?false:!reply.stop;if(runtime.needsHuman)notifySeller();runtime.lastFingerprint=fingerprint(transcript());await remember(runtime.active);}
 ...
 async function processCurrent(){
  if(runtime.busy||runtime.sending||!runtime.active||!runtime.contactKey)return;const {assistantEnabled}=await chrome.storage.local.get('assistantEnabled');if(assistantEnabled===false)return;const items=transcript(),last=items.at(-1);if(!last||last.role!=='cliente')return;const mark=fingerprint(items);runtime.lastFingerprint=mark;runtime.busy=true;render();
