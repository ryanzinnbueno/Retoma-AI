(()=>{
 ...
 function openPanel(){let panel=document.querySelector('#retoma-panel');if(panel){panel.remove();document.querySelector('#retoma-launcher').hidden=false;return;}runtime.renderKey='';panel=document.createElement('aside');panel.id='retoma-panel';panel.innerHTML=`<header><img src="${icon}" alt="Símbolo Retoma"><div><b>retoma</b><small id="retoma-contact">Abra uma conversa</small></div><button type="button" aria-label="Fechar">×</button></header><div class="retoma-panel-body"><div class="retoma-status" id="retoma-status">Conectando…</div><div id="retoma-dynamic"></div><a class="retoma-dashboard" href="${escape(runtime.dashboardUrl||'https://retoma-ai.vercel.app')}" target="_blank">Abrir central de atendimento <span>↗</span></a><footer><span></span>Versão 0.6.3 · mantenha esta aba aberta</footer></div>`;document.body.appendChild(panel);document.querySelector('#retoma-launcher').hidden=true;panel.querySelector('header button').onclick=openPanel;render();}
