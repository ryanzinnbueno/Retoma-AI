...
test('extension link activates, auto replies once, acknowledges delivery and pauses for seller',async()=>{
 ...
 response=await extensionChat.POST(makeRequest({...base,mode:'auto',requestId:crypto.randomUUID()}));data=await response.json();assert.equal(response.status,200);assert.match(data.reply.text,/fachada/i);assert.ok(data.reply.parts.length>=2);const sentReply=data.reply.text;
 ...
 response=await extensionChat.POST(makeRequest({mode:'outbox'}));data=await response.json();assert.equal(response.status,200);assert.equal(data.items[0].pending.sender,'vendedor');assert.equal(data.items[0].contactKey,'5511999999999');
 response=await extensionChat.POST(makeRequest({mode:'sent',contactKey:base.contactKey,contactName:base.contactName,requestMessageId:data.items[0].pending.requestMessageId,messages:[]}));assert.equal(response.status,200);
 saved=await state.load(owner);lead=saved.data.leads.find(l=>l.externalId==='whatsapp-web:5511999999999');assert.equal(lead.messages.at(-1).role,'vendedor');assert.equal(lead.messages.at(-1).delivery,'sent');assert.equal(lead.ai,false);assert.equal(lead.pendingExtensionReply,undefined);
...
test('server timestamps closings; confirmed amounts retain closing date; client cannot forge dates',async()=>{
 ...
 const {periodResults,localDate,periodRange,shortMessages,conversationalMessages,dedupeRepeatedText}=await import(await compile('app/conversation-tools.ts'));
 ...
 assert.deepEqual(conversationalMessages('Primeira orientação. Segunda pergunta?'),['Primeira orientação.','Segunda pergunta?']);
 assert.deepEqual(conversationalMessages('Resposta curta.'),['Resposta curta.']);
 assert.equal(dedupeRepeatedText('Olá! Como posso ajudar?Olá! Como posso ajudar?'),'Olá! Como posso ajudar?');
});
