...
test('extension link activates, auto replies once, acknowledges delivery and pauses for seller',async()=>{
 ...
 response=await extensionChat.POST(makeRequest({...base,mode:'auto',requestId:crypto.randomUUID(),messages:[...base.messages,{role:'cliente',text:sentPart}]}));data=await response.json();assert.equal(data.noReply,true);
