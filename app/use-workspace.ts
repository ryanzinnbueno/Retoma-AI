...
export function useWorkspace(){
 ...
 const ref=useRef(state),queue=useRef<Promise<unknown>>(Promise.resolve()),savingRef=useRef(false);
 ...
 useEffect(()=>{reload().catch(e=>setError(e.message));const timer=setInterval(()=>{if(document.visibilityState==='visible'&&!savingRef.current)reload().catch(()=>{});},7000);return()=>clearInterval(timer);},[]);
 function mutate(fn:(data:Workspace)=>Workspace,retryConflict=false){savingRef.current=true;setSaving(true);setError('');queue.current=queue.current.catch(()=>{}).then(async()=>{
