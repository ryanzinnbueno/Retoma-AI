...
export async function POST(req:Request){let owner='',eventId='';try{
 ...
 const at=new Date().toISOString(),before=lead.messages.length;lead=mergeImportedMessages(lead,items,at);lead={...lead,...(contactAvatar?{avatar:contactAvatar}:{}),name:lead.nameEdited?lead.name:name,service:typeof body.subject==='string'&&body.subject.trim()?body.subject.trim().slice(0,160):lead.service,consent:true,status:'Conversando'};
