...
export async function POST(req:Request){let owner='',eventId='';try{
 owner=await authenticate(req);const body=await readBody(req);const mode=['status','activate','auto','sent','seller','pause'].includes(body.mode)?body.mode:'status';
 ...
 if(mode==='sent'){
  ...
  const next:Lead={...lead,lastAutoReplyTo:pending.requestMessageId,pendingExtensionReply:undefined,due:pending.stop?'Não contatar':lead.needsHuman?'Ação do vendedor · IA disponível':'Aguardando cliente',messages:[...lead.messages,{id:'wa-ai-'+crypto.randomUUID(),role:'ia',text:pending.text,provider:'Gemini',createdAt:at,delivery:'sent'}],history:[...lead.history,pending.stop?'Resposta final enviada e acompanhamento encerrado.':lead.needsHuman?'Resposta enviada; vendedor alertado e IA continua disponível.':'Resposta automática enviada pela extensão.']};snapshot.data.leads=snapshot.data.leads.map(l=>l.id===next.id?next:l);await commit(owner,snapshot.revision,snapshot.data);return respond({...state(next),acknowledged:true});
 }
 ...
 const pending={requestMessageId:last.id,text:decision.text,parts:shortMessages(decision.text),summary:decision.summary,needsHuman,reason,stop,createdAt:at};
