...
function Avatar({ lead, small = false }: { lead: Lead; small?: boolean }) {
  return (
    ...
    >
      {lead.avatar ? <img src={lead.avatar} alt="" /> : initials(lead.name)}
    </span>
  );
}
...
export default function Dashboard() {
  ...
  const real = !!lead.externalId?.startsWith('whatsapp-web:'),
    realCount = leads.filter((l) =>
      l.externalId?.startsWith('whatsapp-web:'),
    ).length;
  ...
  async function deleteConversation() {
    ...
    try {
      ...
      await workspace.removeLead(lead.id);
      setActive(remaining[0]?.id || 0);
      setDeleteConfirm(false);
      notify(
        'Conversa excluída',
        `O histórico de ${lead.name} foi removido do Retoma.`,
      );
    } catch (error) {
      setAIError(
  ...
  async function sendRealMessage() {
    ...
    try {
      ...
      await queueExtensionMessage(lead.id, text);
      setDraft('');
      await workspace.reload();
      notify(
        'Mensagem enviada para a extensão',
        'O WhatsApp Web fará o envio assim que localizar esta conversa.',
      );
    } catch (e) {
      setAIError(
        e instanceof Error
    ...
    <Toaster>
      <div className="retoma-shell">
        ...
        >
          ...
          <div className="nav-row">
            ...
            <span className="connection-note">
              <ShieldCheck size={14} />
              Extensão disponível para teste
            </span>
          </div>
          <div
          ...
          <div className={'simulation-banner ' + (real ? 'is-live' : '')}>
            ...
            <span>
              {real
                ? 'Conversa recebida da extensão. Mensagens automáticas são enviadas pelo WhatsApp Web enquanto ele estiver aberto.'
                : 'Demonstração com dados fictícios. Conecte a extensão para acompanhar conversas reais.'}
            </span>
          </div>
              ...
              <section className="chat-surface">
                <header className="conversation-header">
                  ...
                  <div className="conversation-person">
                    <h2>{lead.name}</h2>
                    <p>
                      {lead.city || lead.company || 'Local não informado'}{' '}
                      <span>•</span>{' '}
                      {real ? 'WhatsApp Web sincronizado' : 'Demonstração'}
                    </p>
                  </div>
                  ...
                  <button
                    className="icon-btn conversation-edit"
                  ...
                  <button
                    ...
                    aria-label={'Excluir conversa com ' + lead.name}
                ...
                <div className="message-stream">
                  ...
                  {lead.ai && (
                    <div className="waiting-note">
                      <span className="dot" />
                      {real
                        ? 'Extensão monitorando novas mensagens'
                ...
                <div className="composer-area">
                      ...
                      <span>
                        {business(config).paused
                          ...
                          : lead.needsHuman
                            ...
                            : real
                              ? 'A extensão responde pelo WhatsApp Web.'
                    ...
                    <form
                      ...
                      onSubmit={(e) => {
                        ...
                        update(lead.id, (l) => ({
                          ...
                          history: [
                            ...l.history,
                            'Vendedor respondeu na simulação.',
                          ],
                        }));
                        setDraft('');
                      }}
                      ...
                      <input
                        ...
                        placeholder={
                          terminal(lead)
                            ? 'Conversa finalizada'
                            : real &&
                                lead.pendingExtensionReply?.sender ===
                                  'vendedor'
                              ? 'Aguardando envio pelo WhatsApp Web…'
                              : real
                                ? 'Escreva para enviar pelo WhatsApp Web…'
                  ...
                  <div className="composer-disclaimer">
                    <ShieldCheck size={12} />
                    {real
                      ? 'A extensão envia com o Chrome e o WhatsApp Web abertos.'
                      : 'Ambiente demonstrativo, sem envio real.'}
                  </div>
                  {real && aiError && (
                        ...
                        {lead.ai
                          ...
                          : lead.optOut
                            ...
                            : terminal(lead)
                              ...
                              : real
                                ? 'Abra esta conversa no WhatsApp Web e clique em Acompanhar com IA.'
                                : 'Escolha esta conversa, revise o contexto e autorize a continuidade do atendimento.'}
                      </p>
                      {lead.ai ? (
                        <button
                          className="btn outline full"
                      ...
                      <div className="followup-sequence">
                        ...
                        <p>
                          {lead.status === 'Conversando'
                            ? 'O teste fica disponível quando a conversa estiver parada.'
                            : !config.followups
                              ? 'Remarketing desativado nas configurações.'
                              : lead.attempts >= config.days.length
                                ? 'Limite de tentativas atingido.'
                                : `A IA usará o produto e apenas materiais confirmados. Próximo contato: ${lead.due}`}
                        </p>
                        <button
                          className="btn outline full"
                        ...
                        >
                          ...
                          {busy
                            ? 'Criando mensagem…'
                            : 'Testar mensagem de remarketing'}
                        </button>
                        <button
                          className="quiet-btn"
                          disabled={busy || !canFollow}
                  ...
                  <TabsContent value="summary" className="copilot-body">
                    <div className="summary-title">
                      ...
                      <div>
                        ...
                        <p>O que a IA conversou e como ficou o atendimento.</p>
                      </div>
                    </div>
                    <div className="summary-item summary-generated">
                      <small>
                        <Sparkles size={14} /> EM POUCAS PALAVRAS
                      </small>
                      <p>{summary.talked}</p>
                    </div>
                    <div className="summary-item summary-interest">
                      <small>
                        <FileText size={14} /> ASSUNTO
                      </small>
                      ...
                      <TableRow key={l.id}>
                        <TableCell>
                          <div className="table-client">
                            ...
                            <div>
                              <b>{l.name}</b>
                              <small>{l.company || 'WhatsApp Web'}</small>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
        ...
        <footer className="product-footer">
          ...
          <span>Beta conectado · mantenha o WhatsApp Web aberto</span>
        </footer>
        ...
        <Dialog open={editClient} onOpenChange={setEditClient}>
          <DialogContent className="retoma-dialog">
            ...
            <DialogDescription>
              Essas informações ajudam a IA a entender este atendimento.
            </DialogDescription>
            <form
              onSubmit={(event) => {
                ...
                update(lead.id, (current) => ({
                  ...
                  history: [
                    ...current.history,
                    'Dados do cliente atualizados pelo vendedor.',
                  ],
                }));
                setEditClient(false);
            ...
            >
              ...
              <button className="btn primary full" type="submit">
                <Check size={17} /> Salvar dados do cliente
              </button>
            </form>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={deleteConfirm} onOpenChange={setDeleteConfirm}>
          <DialogContent className="retoma-dialog delete-dialog">
            <div className="dialog-emblem delete-emblem">
              <Trash2 size={23} />
            </div>
            <DialogTitle>Excluir conversa com {lead.name}?</DialogTitle>
            <DialogDescription>
              As mensagens, o resumo e o acompanhamento serão removidos do
              Retoma. Isso não apaga a conversa dentro do WhatsApp.
            </DialogDescription>
          ...
          <DialogContent className="retoma-dialog activation-dialog">
            ...
            <form
              onSubmit={(e) => {
                ...
                try {
                  ...
                  setSideTab('follow');
                  notify(
                    'Acompanhamento ativado na simulação',
            ...
            >
              ...
              <div className="activation-checklist">
                ...
                <span>
                  <Check size={16} />
                  Pausar e resumir quando o vendedor for necessário.
                </span>
                <span>
                  <Check size={16} />
              ...
              <label className="consent-line">
                ...
                <span>
                  Confirmo, nesta simulação, que este cliente autorizou o
                  contato e que o contexto foi revisado.
                </span>
              </label>
              <button
                className="btn primary full"
              ...
              <p className="demo-footnote">
                Ativação demonstrativa. Não conecta, lê ou envia mensagens no
                WhatsApp.
              </p>
            </form>
          </DialogContent>
