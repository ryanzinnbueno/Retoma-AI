...
export function conversationalMessages(text:string){
 ...
 const units=trimmed.split(/\n+|(?<=[.!?])\s+(?=[A-ZÁÀÂÃÉÊÍÓÔÕÚÇ0-9])/u).map(part=>part.trim()).filter(Boolean).flatMap(part=>part.length>220?shortMessages(part):[part]);
 if(units.length<=4)return units;
 return [...units.slice(0,3),units.slice(3).join(' ')];
}
